...
public $add_new;
    ...
    public function deleteElevator($delete_id)
    {
        $data = Elevator::where("id", $delete_id);
        $data->delete();
 

        pnotify()->addWarning('Lift ge-archiveerd');
        return redirect()->to('/company/elevators');

    }



    public function archiveElevator($delete_id)
    {
        $data = Elevator::where("id", $delete_id);
    ...
    public function DeArchiveElevator($delete_id)
    {
        $data = Elevator::where("id", $delete_id);
        ...
        pnotify()->addWarning('Lift ge-archiveerd');
        return redirect()->to('/company/elevator/show/' . $delete_id);
...
public function savemaintenanceContractAction()
{
    ...
    $maintenancie = MaintenancyContracts::updateOrCreate(
        ['id' => $this->maintenance_contract_id, ], 
        ['enddate' => $this->maintenance_contract_enddate, 
        'begindate' => $this->maintenance_contract_begindate, 
        'option1' => $this->maintenance_contract_option1, 
        'option2' => $this->maintenance_contract_option2, 
        'option3' => $this->maintenance_contract_option3, 
        'type_id' => $this->maintenance_contract_type_id, 
        'elevator_id' => $this->elevator->id,
         'maintenancy_companie_id' => $this->maintenance_contract_companie_id
...
public function saveMaintenance()
{
 ...
 
    {
        ...
        $filename = preg_replace('/\s+/', '_', $this
            ...
            $uploadedFile = $this->maintenance_attachment;
    ...
    $this->maintenance_remark =NULL;
    $this->maintenance_planned_at =NULL;
    $this->maintenance_status_id = NULL;
    $this->maintenance_executed_datetime = NULL;
...
public function saveMJOB()
{
    ...
    $data = Upload::updateOrCreate(["type_id" => 99, "filename" => $filename, "title" => "Meerjarenbegroting " . $this->mjob_beginyear . ' - ' . $this->mjob_endyear, "path" => $directory, "group_id" => 1,
    ...
    "relation_id" => $this
        ...
        //Reset form fields
        $this->mjob_attachment = null;
        $this->mjob_beginyear = null;
        $this->mjob_endyear = null;
...
public function saveInspection()
{
    ...
    $validatedData = $this->validate(
        ["inspection_executed_datetime" => "required", 
    ...
    $inspection = Inspection::updateOrCreate(
        ...
        ["executed_datetime" => $this->inspection_executed_datetime, 
        "end_date" => $this->inspection_end_date, 
        "status_id" => $inspection_status_id, 
        "remark" => $this->inspection_remark, 
        "elevator_id" => $this
        ->elevator->id, ]);

        //Update de lift met de status IS van de inspoectio
        Elevator::find($this->elevator->id)->update(array('inspection_status_id' => $inspection_status_id));  // update the record in the DB.
    //     $filename = $this
    //     ->attachmentDocument
    //     ->getClientOriginalName();
    ...
    if ($this->attachmentCertification != $from_database->certification )
    {
        ...
        $uploadedFile = $this->attachmentCertification;
        ...
        $directory = $this
            ->elevator
            ->address->customer_id . "/" . $this
            ->elevator->id . "/inspections/" . $inspection->id;
        ...
        $update->update([
        //
        'certification' =>  $directory. "/".$filename
        ]);
    ...
    if ($this->attachmentDocument != $from_database->document )
    {
        ...
        $uploadedFile = $this->attachmentDocument;
...
public function deleteInspectionDocument($type,$inspection_id)
{
    ...
    public function downloadBase64($base64String,$filename){
