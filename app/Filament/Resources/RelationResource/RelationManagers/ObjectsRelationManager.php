...
class ObjectsRelationManager extends RelationManager
{
      ...
      Wizard::make([
          ...
          ->schema([
            ...
            ->schema([
              TextInput::make("stopping_places")
                ->label("Aantal stopplaatsen")
                ->integer(),
              TextInput::make("carrying_capacity")
                ->label("Draaggewicht ")
                ->integer(),
              ...
              Checkbox::make("fire_elevator")
                ->inline(true)
                ->label("Brandweerlift"),
              Checkbox::make("stretcher_elevator")
                ->inline(true)
                ->label("Brancardlift "),
            ]),
          ])
        ...
        Step::make("Toewijzing")->schema([
          ...
          ->schema([
            ...
            Select::make("department_id")
              ->label("Afdeling")
              ->options(
                fn() => $this->ownerRecord?->departments?->pluck(
                  "name",
                  "id",
                ) ?? [],
              )
              ->searchable()
              ->visible(function ($record, callable $get) {
                $object_data = ObjectType::where(
                  "id",
                  $get("type_id"),
                )->first();
                if ($object_data?->visibility) {
                  return in_array("Afdeling", $object_data?->visibility)
                    ? true
                    : false;
                } else {
                  return false;
        ...
        Step::make("Opmerking")->schema([
          Textarea::make("remark")
            ...
            ->autosize()
            ->hint(
              fn($state, $component) => "Aantal karakters: " .
  ...
  public function table(Table $table): Table
  {
    return $table
      ->groups([
        Group::make("type.name")->titlePrefixedWithLabel(false)->label("Merk"),

        Group::make("brand")->titlePrefixedWithLabel(false)->label("Merk"),

        Group::make("model")->titlePrefixedWithLabel(false)->label("Model"),
      ...
      ->columns([
        ...
        ViewColumn::make("energy_label")
          ->view("filament.tables.columns.energylabel")
          ->label("Energylabel")
      ...
      ->headerActions([
        Tables\Actions\CreateAction::make()
          ...
          ->modalDescription(
            "Koppel vaste objecten aan een locatie of een medewerker. Wil je objecten koppelen aan aan een medewerker of locatie? Maak dan eerst een medewerker of locatie aan.",
          )
          ->label("Toevoegen")
      ...
      ->actions([
        ...
        Tables\Actions\EditAction::make()
          ...
          ->modalDescription(
            "Pas de bestaande object aan door de onderstaande gegevens zo volledig mogelijk in te vullen.",
          )
          ->tooltip("Bewerken"),
      ...
      ->bulkActions([
        ExportBulkAction::make()
          ->label("Exporteren")
          ->fileName(
            fn(Export $export): string => "objecten-{$export->getKey()}",
          )
